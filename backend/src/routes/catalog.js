...
app.get("/brands", async (c) => {
  const { results } = await c.env.DB.prepare(
    `SELECT b.id, b.name, b.image_key AS imageKey,
            ...
            (SELECT COUNT(*) FROM models m WHERE m.brand_id = b.id) AS modelCount,
...
app.get("/brands/:brandId/families", async (c) => {
  const { results } = await c.env.DB.prepare(
    `SELECT f.id, f.name,
            (SELECT COUNT(*) FROM models m WHERE m.family_id = f.id) AS modelCount,
            (SELECT m.category FROM models m WHERE m.family_id = f.id
             ORDER BY m.display_order, m.name LIMIT 1) AS category,
            (SELECT MIN(p.price) FROM model_issue_types p
             JOIN models m ON m.id = p.model_id
             WHERE m.family_id = f.id) AS minPrice
...
app.get("/brands/:brandId/models", async (c) => {
  ...
  const cols = `m.id, m.name, m.category, m.image_key AS imageKey,
                (SELECT MIN(p.price) FROM model_issue_types p WHERE p.model_id = m.id) AS minPrice`;
  const stmt = familyId
    ? c.env.DB.prepare(
        `SELECT ${cols} FROM models m WHERE m.brand_id = ? AND m.family_id = ? ORDER BY m.display_order, m.name`
      ).bind(brandId, familyId)
    : c.env.DB.prepare(
        `SELECT ${cols} FROM models m WHERE m.brand_id = ? ORDER BY m.display_order, m.name`
      ).bind(brandId);
